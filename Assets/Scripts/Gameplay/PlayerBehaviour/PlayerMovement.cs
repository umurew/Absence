...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        Vector2 moveInput = _inputService.PlayerActions.Move.ReadValue<Vector2>();
        ...
        if (noclip)
        {
            ...
            if (_inputService.PlayerActions.Jump.IsInProgress())
            {
            ...
            else if (_inputService.PlayerActions.Crouch.IsInProgress())
            {
                Vector3 up = cameraTransform.up;
                up.Normalize();
        ...
        if (characterController.isGrounded)
        {
            ...
            if (crouchAllowed && _inputService.PlayerActions.Crouch.IsInProgress())
            {
                isCrouching = true;
            ...
            if (jumpAllowed && _inputService.PlayerActions.Jump.WasPressedThisFrame())
            {
                isCrouching = false;
        ...
        bool isSprinting = sprintAllowed && _inputService.PlayerActions.Sprint.IsInProgress();
